...
public class Test {
    ...
    private static <E> void testIterating(String desc, Collection<? extends E> collection,
                                          HashSet<? extends E> predicate) {

        FilterList<E> filterList = new FilterList<E>(collection, predicate);

        System.out.print("--- " + desc + " ---"
                + "\nList:      " + Arrays.toString(filterList.toArray())
                + "\nPredicate: " + filterList.getPredicate().toString()
                + "\nIterating: ");
    ...
    private static <E> void testRemovingObject(String desc, Collection<? extends E> collection,
                                               HashSet<? extends E> predicate, Object obj) {
        ...
        System.out.println("--- " + desc + " ---"
                ...
                + (filterList.elementAllowed(obj) ? ", not in the predicate." : ", in the predicate."));
        boolean check = filterList.remove(obj);
        System.out.println((check ? "Object is removed." : "Object is not removed.")
                + "\nResult list: " + Arrays.toString(filterList.toArray()) + "\n");
    }

    private static <E> void testRemovingAtIndex(String desc, Collection<? extends E> collection,
                                                HashSet<? extends E> predicate, int index) {
        ...
        System.out.println("--- " + desc + " ---"
                + "\nList:        " + Arrays.toString(filterList.toArray())
                + "\nPredicate:   " + filterList.getPredicate().toString()
                + "\nTrying to remove at index: " + index + " - Element: " + filterList.get(index).toString()
                + (filterList.elementAllowedAt(index) ? ", not in the predicate." : ", in the predicate."));
        try {
            E elem = filterList.remove(index);
            System.out.println("Removed element: " + elem.toString());
        } catch (ElementInPredicateException e) {
            System.out.println(e.getMessage());
        }
        System.out.println("Result list: " + Arrays.toString(filterList.toArray()) + "\n");
    }

    private static <E> void testAddingElement(String desc, Collection<? extends E> collection,
                                              HashSet<? extends E> predicate, E element) {
        ...
        System.out.println("--- " + desc + " ---"
                ...
                + (filterList.elementAllowed(element) ? ", not in the predicate." : ", in the predicate."));
        boolean check = filterList.add(element);
    ...
    private static <E> void testAddingAtIndex(String desc, Collection<? extends E> collection,
                                              HashSet<? extends E> predicate, int index, E element) {
    ...
    public static void main(String[] args) {
        ...
        testIterating("Test 1. List and predicate are both Integer.",
                ...
                new HashSet<Integer>(Arrays.asList(new Integer[]{5, 9, 4})));
        ...
        testIterating("Test 7. List and predicate are both String.",
                ...
                new HashSet<String>(Arrays.asList(new String[]{"e", "ab"})));
        ...
        testIterating("Test 9. List is Integer, predicate is Number.",
                ...
                new HashSet<Number>(Arrays.asList(new Number[]{5., 9., 4})));
